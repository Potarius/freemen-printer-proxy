 ...
 */
function updateSavedPrinter(originalIp, updates) {
  const config = loadConfig();
  const index = config.savedPrinters.findIndex(p => p.ip === originalIp);
  
  if (index === -1) {
    throw new Error(`Imprimante ${originalIp} non trouvée`);
  }
  
  config.savedPrinters[index] = {
    ...config.savedPrinters[index],
    ...updates,
    updatedAt: new Date().toISOString()
  };
